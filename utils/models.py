...
def load_model(model_filepath: str, torch_dtype: torch.dtype = torch.float16):
    ...
    if round_config["use_lora"]:
        ...
        logging.info(
            "loading the base model (before LORA) from {}".format(base_model_filepath)
        )
        ...
        logging.info(
            "loading the LORA adapter onto the base model from {}".format(
                fine_tuned_model_filepath
            )
        )
        ...
        fine_tuned_model_filepath = os.path.join(model_filepath, "fine-tuned-model")
        logging.info(
            "Loading full fine tune checkpoint into cpu from {}".format(
                fine_tuned_model_filepath
            )
        )
        model = AutoModelForCausalLM.from_pretrained(
