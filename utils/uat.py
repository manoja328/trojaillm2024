...
def get_embedding_weight(language_model):
    ...
    for module in language_model.modules():
        if isinstance(module, torch.nn.Embedding):
            if module.weight.shape[0] == language_model.model.embed_tokens.weight.shape[0]: # only add a hook to wordpiece embeddings, not position embeddings
...
class UAT():
    ...
    def add_hooks(self,language_model):
        for module in language_model.modules():
            if isinstance(module, torch.nn.Embedding):
                if module.weight.shape[0] == language_model.model.embed_tokens.weight.shape[0]:
    ...
    def _predict_single_target(self, target, tokenizer, model, num_generate, num_optim_tokens,
        ...
        for _ in range(num_generate):
            ...
            #
            total_vocab_size = model.model.embed_tokens.weight.shape[0]  # total number of subword pieces in the GPT-2 model
            ...
            for i in range(num_steps):  # this many updates of the entire trigger sequence
                for token_to_flip in range(0, trigger_token_length): # for each token in the trigger
                    ...
                    # candidates = random_attack(embedding_weight, [token_to_flip], num_candidates=1)
